 ...
 */
if (!class_exists('Fresh_Blog_Lite_Theme_Info')) {

  class Fresh_Blog_Lite_Theme_Info
  {
    const NOTICE_USER_META_KEY   = 'fresh_blog_lite_notice_dismissed';
    const NOTICE_CSS_CLASS       = 'fresh-blog-lite-notice';
    const NOTICE_REAPPEAR_DAYS   = 7; // Days after which the notice reappears

    /**
     * Hold the class instance
     *
     * @var Fresh_Blog_Lite_Theme_Info|null
     */
     ...
     *
     * @return Fresh_Blog_Lite_Theme_Info
     */
    ...
    private function __construct()
    {
      ...
      add_action('wp_ajax_fresh_blog_lite_notification_dismissal', array($this, 'theme_notification_ajax_callback'));
    }
    ...
    public function __clone()
    {
      _doing_it_wrong(
        __FUNCTION__,
        esc_html__('Cloning is not allowed.', 'fresh-blog-lite'),
        '1.0.0'
      );
    }
    ...
    public function __wakeup()
    {
      _doing_it_wrong(
        __FUNCTION__,
        esc_html__('Unserializing instances is not allowed.', 'fresh-blog-lite'),
        '1.0.0'
      );
    }
    ...
    {
      add_theme_page(
        esc_html__('Theme Info', 'fresh-blog-lite'),
        esc_html__('Theme Info', 'fresh-blog-lite'),
        'edit_theme_options',
    ...
    public function enqueue_notice_script()
    {
      ...
      wp_localize_script(
        'fresh-blog-lite-admin-notice',
        'fresh_blog_lite_notice',
        array(
          'nonce'        => wp_create_nonce('fresh_blog_lite_notification_nonce'),
          'action'       => 'fresh_blog_lite_notification_dismissal',
    ...
    {
      check_ajax_referer('fresh_blog_lite_notification_nonce', 'nonce');

      if (!current_user_can('edit_theme_options')) {
        wp_send_json_error(
          array('message' => esc_html__('Unauthorized access.', 'fresh-blog-lite')),
          403
        );
      }
      ...
      wp_send_json_success(
        array('message' => esc_html__('Notice dismissed successfully.', 'fresh-blog-lite'))
      );
    }
  }

  // Instantiate singleton
  Fresh_Blog_Lite_Theme_Info::get_instance();
}
